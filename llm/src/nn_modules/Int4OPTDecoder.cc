...
Matrix3D<float> Int4OPTDecoder::prepare_decoder_attention_mask(int length, int past_length) {
    PROFILE_START("Int4OPTDecoder::prepare_decoder_attention_mask");
    ...
    PROFILE_END("Int4OPTDecoder::prepare_decoder_attention_mask");
    return causal_attention_mask;
}

Matrix3D<float> Int4OPTDecoder::get_position_embed(int sql_length, int past_length) {
    PROFILE_START("Int4OPTDecoder::get_position_embed");
    ...
    PROFILE_END("Int4OPTDecoder::get_position_embed");
    return pos_embeds;
}

Int4OPTDecoder::Int4OPTDecoder(std::string param_path, const struct model_config config) {
    ...
    for (int layer_idx = 0; layer_idx < config.num_layers; layer_idx++) {
        ...
        Int4OPTDecoderLayer layer = Int4OPTDecoderLayer(path, config, layer_idx);
...
// OPTDecoder:
struct Int4OPTDecoder_output Int4OPTDecoder::forward(const struct Int4OPTDecoder_input &input) {
    ...
    for (int i = 0; i < this->layers.size(); i++) {
        if (!input.has_past_keys_values) {
            struct Int4OPTDecoderLayer_input l_i = {hidden_states, causal_attention_mask};
            struct Int4OPTDecoderLayer_output l_o = this->layers[i].forward(l_i);
            ...
            struct Int4OPTDecoderLayer_input l_i = {hidden_states, causal_attention_mask, input.past_keys[i],
                                                    input.past_values[i]};
            struct Int4OPTDecoderLayer_output l_o = this->layers[i].forward(l_i);
            hidden_states = l_o.hidden_states;
            past_keys.push_back(l_o.past_key_value.first);
    ...
    struct Int4OPTDecoder_output output = {last_hidden_states, past_keys, past_values};
